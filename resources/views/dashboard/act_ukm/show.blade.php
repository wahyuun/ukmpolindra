        ...
        <div class="row justify-content-center">
            <div class="col-lg-10 col-xl-9">
                <div class="card card-lg">
                    @if($kegiatan->status==2 && empty($kegiatan->komentar))
                    <div class="alert alert-danger text-center" role="alert">
                        <h4 class="alert-heading text-danger fs-3 d-block">ANDA BELUM MENGISI KOMENTAR!</h4>
                        <p><a href="{{ route('kegiatan.edit',$kegiatan->slug) }}">"Isi Komentar"</a> untuk menindaklanjuti status <span class="fw-bold text-danger">ditolak</span> (Bersifat wajib!)</p>
                    </div>
                    @endif

                    <div class="page-pretitle mt-3 d-flex justify-content-between">
                        <a class="ms-3" href="{{ route('kegiatan.index') }}" title="Kembali"
                    ...
                    <div class="dropdown" title="Status action" data-bs-toggle="tooltip" data-bs-placement="bottom">
                        ...
                        <form action="{{ url('/act-kegiatanPending?pending='.$kegiatan->slug) }}" method="POST">
                            @csrf
                            @method('PATCH')
                        ...
                        <form action="{{ url('/act-kegiatanDiterima?diterima='.$kegiatan->slug) }}" method="POST">
                            @csrf
                            @method('PUT')
                        ...
                        <form action="{{ url('/act-kegiatanDitolak?ditolak='.$kegiatan->slug) }}" method="POST">
                            @csrf
                            @method('PUT')
            ...
            <div class="card-body markdown">
                <h1 id="whos-that-then" class="text-center">{{ $kegiatan->nama_kegiatan }}</h1>
                <h3 class="card-title mt-5">Status permohonan:
                    ...
                    @if($kegiatan->komentar > 0)
                    <a title="Terdapat komentar" data-bs-toggle="tooltip"
                    data-bs-placement="bottom" class="position-absolute" href="{{ url('/act-komentar?act-ditolak='.$kegiatan->slug) }}"><img style="margin-top: -35px; margin-left:-15px;" width="70" src="img/message.gif" alt=""></a>
                    @else
            ...
            <p><small class="text-muted">
                ...
                {{ \Carbon\Carbon::parse($kegiatan->tgl_kegiatan)->isoFormat('dddd, DD MMMM Y HH:mm:ss a'); }}
            </small></p>
