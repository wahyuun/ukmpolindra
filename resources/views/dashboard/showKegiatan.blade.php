        ...
        <div class="col-12">
            <div class="card">
                <div class="card-table table-responsive p-3">
                    <table class="table" id="myDashboardKegiatan">
                        <thead>
                            <tr>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $act->ukm->nama_ukm }}</td>
                                    <td>{{ $act->nama_kegiatan }}</td>
                                    <td>{{ Str::limit(strip_tags($act->deskripsi), 30)  }}</td>
                                    <td>{{ $act->tgl_kegiatan }}</td>
                                    <td>{{ \Carbon\Carbon::parse($act->created_at)->isoFormat('dddd, DD MMMM Y HH:mm:ss a') }}</td>
                                    <td>{{ \Carbon\Carbon::parse($act->updated_at)->isoFormat('dddd, DD MMMM Y HH:mm:ss a') }}</td>
                                    <td>{{ $act->file ?? 'Data kosong' }}</td>
                                    <td>
                                        @if ($act->status == 0)
                                        ...
                                        @elseif($act->status == 2)
                                        @php
                                        $komentar = url('/act-komentar?act-ditolak='.$act->slug);
                                        if ($act->komentar) {
                                            if ($act->ukm->status != 0) {
                                                echo '<p class="badge bg-danger">Ditolak</p>
                                                <a href="'.$komentar.'" class="d-block link-danger" title="Komentar"
